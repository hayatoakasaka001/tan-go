const STORAGE_KEY = "tan-go-state-v1";
const SESSION_KEY = "tan-go-session-v1";
const WORDS_KEY = "tan-go-words-v1";
...
function resetWords() {
  if (!confirm("読み込んだ単語データを削除して、仮データに戻しますか？")) return;
  words = DEFAULT_WORDS;
...
function normalizeLevel(value) {
  ...
  if (!raw) return "A";
  const match = raw.match(/[ABCＡＢＣ]/i);
  if (match) return match[0].toUpperCase().replace("Ａ", "A").replace("Ｂ", "B").replace("Ｃ", "C");
  return raw;
}
...
function renderSettings() {
  ...
  screen(`
    ...
    <div class="button-stack">
      ...
      <button class="secondary-button" data-action="reset-words" ${isUsingImportedWords ? "" : "disabled"}>仮データに戻す</button>
